...
# entrypoint
# programmaticAPI
mkdir -p ./distribution/entrypoint/programmaticAPI/ && echo "module.exports = require('../../source/script.js')" >> ./distribution/entrypoint/programmaticAPI/index.js
# cli
if [ "$copyCli" = true ] ; then
  mkdir -p ./distribution/entrypoint/cli && echo "#\!/usr/bin/env node\nmodule.exports = require('../../source/scriptManager/clientInterface/commandLine.js')" >> ./distribution/entrypoint/cli/index.js
fi

# source
yarn run babel --out-dir ./distribution/source --config-file "./configuration/babel.config.js" --copy-files  "./source"
# script
